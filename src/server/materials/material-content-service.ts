...
import { resolveStoredMaterialPath } from "./file-validation";

import type { StoredMaterialContent } from "./material-content-response";
export type { StoredMaterialContent } from "./material-content-response";
...
export async function getPublicProfileMaterialContent(slug: string) {
  const context = await requirePublicAgentContext(slug);
  const result = await getPool().query<{ profileMaterialId: string | null }>(
    `SELECT profile_material_id AS "profileMaterialId" FROM agent_settings WHERE owner_id=$1`,
    [context.ownerId],
  );
  const materialId = result.rows[0]?.profileMaterialId;
  if (!materialId) throw new AppError("PUBLIC_SOURCE_NOT_FOUND", "The public source was not found.", 404);
  return getPublicMaterialContent(slug, materialId);
}
