...
async function readRequestedSources(tarBytes: Uint8Array, manifestFiles: Map<string, { size: number; contentHash: string }>, requestedPaths: Set<string>) {
  ...
  try {
    ...
    for (const requestedPath of requestedPaths) {
      if (!seen.has(requestedPath) || !sources.has(requestedPath)) {
...
export async function readRepositoryArtifactEvidence(
  ...
  const sources = await readRequestedSources(tarBytes, manifestFiles, requested);
  return {
    eligibleFileCount: manifest.files.length,
    manifestPaths,
    sources,
    artifactSkipped: manifest.skipped,
  };
}
