...
async function loadAgentOrNotFound(slug: string, locale: "en" | "zh-CN") {
  try {
    return await loadPublicAgentBySlug(slug, locale);
  } catch (error) {
...
export default async function PublicAgentPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const projection = await loadAgentOrNotFound(slug, locale);
