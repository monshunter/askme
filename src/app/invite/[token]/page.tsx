...
export default async function InvitationPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="invitation-page"><Link className="wordmark" href="/">Askme <span aria-hidden="true">职问</span></Link><section className="invitation-card"><div className="admin-empty large"><ShieldCheck size={42} /><h1>{t("invite.unavailable")}</h1><p>{t("invite.unavailableCopy")}</p><Link className="primary-button" href="/login">{t("invite.return")}</Link></div></section></main>;
}
