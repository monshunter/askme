...
export default async function AgentsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const query = parseAgentListQuery(parameters); const [data, locale] = await Promise.all([listAdminAgents(query), getRequestLocale()]);
  return <AgentsClient initialPage={JSON.parse(JSON.stringify(data))} initialFilters={query} locale={locale} />;
}
