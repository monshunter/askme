...
import { getCandidateRepositoryDossier } from "@/server/repositories/dossier-review-service";
...
export async function GET(request: NextRequest, context: { params: Promise<{ repositoryId: string }> }) {
  ...
  try {
    ...
    return apiData(await getCandidateRepositoryDossier(getPool(), user.id, repositoryId), id);
  } catch (error) {
