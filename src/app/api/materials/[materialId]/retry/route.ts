...
import { retryMaterial } from "@/server/materials/material-service";
import { requireResourceId } from "@/server/resource-id";

export async function POST(request: NextRequest, context: { params: Promise<{ materialId: string }> }) {
  ...
  try {
    ...
    return apiData(await retryMaterial(user.id, materialId, id), id);
  } catch (error) {
