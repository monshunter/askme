...
import { Bot, Box, CheckCircle2, Cpu, Database, HeartPulse, LoaderCircle, Mail, Save, Send, ServerCog, ShieldCheck } from "lucide-react";
...
type Settings = {
  health: {
    ...
    runner: { status: "ready" | "stale" | "missing"; runnerId: string | null; version: string | null; imageDigest: string | null; artifactReady: boolean; boxliteReady: boolean; safeErrorCode: string | null; lastSeenAt: string | null };
    artifact: { status: "ready" | "degraded" };
    boxlite: { status: "ready" | "degraded" };
  ...
  analysisUsage: { runs: number; completed: number; failed: number; cancelled: number; toolCalls: number };
  policies: { publicSessionHourlyLimit: number; publicChatMinuteLimit: number; negativeFeedbackAutoFlag: boolean };
};
...
export function SettingsClient({ initialSettings, locale }: { initialSettings: Settings; locale: Locale }) {
  ...
  const runnerDetail = settings.health.runner.lastSeenAt ? `${settings.health.runner.version} · ${new Date(settings.health.runner.lastSeenAt).toLocaleString(locale === "zh-CN" ? "zh-CN" : "en-US", { timeZone: "UTC" })}${settings.health.runner.safeErrorCode ? ` · ${settings.health.runner.safeErrorCode}` : ""}` : t("admin.health.noHeartbeat");
