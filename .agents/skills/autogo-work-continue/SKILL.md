...
description: "从 Git、代码、测试和持久制品恢复真实状态并继续工作；在用户要求继续、显式调用 autogo-work-continue、会话或 Worktree 切换，或记录状态可能与实际代码漂移时使用。"
---
# autogo-work-continue
## 目标
从 Git、`PROGRESS.md`、正式 Plan、事实 owner、测试和环境恢复同一次 run 的真实状态，选择未完成工作并持续推进；历史摘要不能覆盖当前事实。
## 输入与发现
- Git 分支、Worktree、Diff、未跟踪文件和最近提交
- 根与受影响组件项目指令
- Standard run 的 `PROGRESS.md` 和正式 Plan；缺失或漂移时从用户目标、Git 和当前事实重建最小真实状态
- Spec、Design、Review、Operation、Deployment record、Decision、Journal 等事实 owner
- 当前测试、环境和运行结果

## 输出与持久制品
- 恢复后的真实状态与差异说明
- 对账后的 Progress、正式 Plan 与对应事实 owner
- 被选择并实际执行的下一 Skill
...
## 失败、重试与幂等
记录无效时以实际仓库重建最小状态；同一错误重复时只有新操作能产生新 Evidence 才重试，否则转 autogo-investigate 或 Rally。
